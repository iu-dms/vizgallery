...
rcplot <- function(score, titletext, captext){
  ggplot(scales_all, aes(x=project, y=score)) + 
    ggdist::stat_halfeye(
      fill=BLUE_50,
      adjust = .5, 
      width = 1.0, 
  ...
  # coord_flip()+
    facet_grid(cols=vars(redcap_event_name), scales="free_x", labeller = labeller(redcap_event_name=new_event_labels))+
  theme_classic()+
  gg_text +
...
plot <- rcplot(scales_all$mspss_score,
              paste0("MSPSS Score (n=", as.numeric(length(na.omit(scales_all$mspss_score))) , ")"),
                "Multidimensional Scale of Perceived Social Support\nMean scores reported; Higher score = Greater Perceived Support")
