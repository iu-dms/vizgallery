...
If you want to download a template R Markdown file that includes the code below, please use this link: [Template .Rmd File](./AE_Vis.Rmd){.external target="_blank"}. The file is set to work with the sample dataset, and would require small edits (as described below) to work with your own input file.
:::


### Data Processing
The code below reads in the data file, processes data into the required formats and calculates additional fields needed for the visualization. Next the data frame is transposed into a structure to better
```{r}
#| code-fold: show
#| code-summary: "Display/Hide AE plot code"
#| eval: false
#| lightbox:
...
This section of could would need to be modified if your severity rating is different than in the example dataset as described above.
:::
...
Next we create cross-sectional datasets for dates needed for the visualization, set functions to create the x-axis limits as well as the facet titles for each ID.
```{r}
#| code-fold: show
#| code-summary: "Display/Hide AE plot code"
#| eval: false
...
![](Example AE Plot.png){.lightbox width=60%}
\
