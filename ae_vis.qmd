...
To replicate the AE plot described below, you will need to provide your data exactly in the structure below. A test dataset is available to download [here](./Sample AE Data_v2.csv){.external target="_blank"} 
...
The code below is an efficient way to load packages at the beginning of a script. The below code includes all of the package dependencies for the AE visualization.
...
We have developed a template R Markdown file that is set to work with the sample dataset, and would only require small edits (as described below) to work with your own input file. The below sections just walk through some of the code hihglights, and may help troubleshooting if you run into any issues modifying the code to fit your use case. You can download the file using this link: [Template .Rmd File](./AE_Vis.Rmd){.external target="_blank"}
:::


### Data Processing Walkthrough

The following sections give a general idea of how the data is processed and used to build the visualization. 
...
::: {.callout-warning title="Modifications"}
This section of could would need to be modified if your severity rating is different than in the example dataset as described above. Also, the **scale_color_manual** line of code in the ggplot2 build (see below) would need to be modified to include more (or less) colors as applicable. See <https://r-graph-gallery.com/38-rcolorbrewers-palettes.html> for more description of the additional color palettes and <https://www.datanovia.com/en/blog/the-a-z-of-rcolorbrewer-palette/> for more ways to work with the *Rcolorbrewer* package.
:::
...
Next we create cross-sectional datasets for all dates needed for the visualization and create functions to calculate the x-axis limits per ID as well as the facet titles for each ID.
```{r}
#| code-fold: false
#| eval: false
#| lightbox:
...
```{r}
#| code-fold: show
#| code-summary: "Display/Hide: HTML to PDF code"
#| eval: false
